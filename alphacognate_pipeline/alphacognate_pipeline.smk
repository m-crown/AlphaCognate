...
rule all:
    input: 
         ...
         all_ranked_ligands = expand(config["output_dir"] + "/filtered_structures/{id}/{id}_cognate_ranking.csv", id = structures),
         ...
         ranked_ligands_structure = expand(config["output_dir"] + "/filtered_structures/{id}_transplants_ranked.cif", id = structures)

rule aggregate_transplants:
    input:
        all_transplant_files = expand(config["output_dir"] + "/filtered_structures/{id}_transplants_ranked.tsv.gz", id = structures), #require the files but dont use in command (for shell max arg limits)
        ...
        #eventually want to modify this aggregation to use the outputs from the ligand ranking step
...
rule transplant_ranked_ligands:
    input:
        structure = config["output_dir"] + "/transplanted_structures/{id}_transplants.cif",
        ranking_file = config["output_dir"] + "/filtered_structures/{id}/{id}_cognate_ranking.csv",
        transplant_df_info = config["output_dir"] + "/transplanted_structures/{id}_transplants.tsv.gz",
        summary = config["output_dir"] + "/transplanted_structures/{id}_structure_summary.tsv.gz"
    output:
        output_cif = config["output_dir"] + "/filtered_structures/{id}_transplants_ranked.cif",
        output_tsv = config["output_dir"] + "/filtered_structures/{id}_transplants_ranked.tsv.gz",
        output_summary = config["output_dir"] + "/filtered_structures/{id}_structure_summary.tsv.gz"
    params:
        top_ligands_only = "--top_ligands_only" if config.get("top_ligands_only") else ""
    shell:
        """python3 bin/nrgrank_transplant.py --structure_file {input.structure} --structure_summary_file {input.summary} --output_summary {output.output_summary} --ranking_file {input.ranking_file} --output_cif {output.output_cif} --output_tsv {output.output_tsv} --transplant_df_info {input.transplant_df_info} {params.top_ligands_only}"""
...
rule rank_ligands:
    ...
    output:
        ...
        config["output_dir"] + "/filtered_structures/{id}/{id}_cognate_ranking.csv"
    params:
        output_prefix = "{id}",
        output_dir = config["output_dir"] + "/filtered_structures/{id}",
    log: config["output_dir"] + "/logs/rank_ligands/{id}_rank.log"
    shell:
        """python3 bin/run_nrgrank.py --target_path_cif {input} --output_prefix {params.output_prefix} --output_dir {params.output_dir}"""
