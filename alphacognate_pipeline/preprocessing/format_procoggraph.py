...
def main():
    ...
    python3 format_procoggraph.py --cath_domain_ownership data/procoggraph_data/cath_pdb_residue_interactions.csv.gz --scores_file data/procoggraph_data/all_parity_calcs.pkl --cognate_ligands data/procoggraph_data/cognate_ligands_df.pkl --bound_entity_descriptors data/procoggraph_data/bound_entity_descriptors.tsv.gz --output_dir data
    ...
    parser.add_argument('--output_dir', type=str, help='output directory for formatted procoggraph data and manifest of pdb ids')
    ...
    cath_single_chain_nonminor_subset_grouped_exploded_merged_ec_grouped.to_csv(f"{args.output_dir}/cath_single_chain_domain_interactions.tsv.gz", index = False, sep = "\t")

    #output a manifest to download the assemblies from modelserver
    pdb_ids = cath_single_chain_nonminor_subset_grouped_exploded_merged_ec_grouped.pdb_id.unique()
    with open(f"{args.output_dir}/procoggraph_pdb_ids.txt", "w") as f:
